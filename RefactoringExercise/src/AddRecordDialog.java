...
public class AddRecordDialog extends JDialog implements ActionListener 
{
	...
	public Container dialogPane() 
	{
		...
		empDetails.add(genderCombo = new JComboBox<String>(this.parent.gender), wrap);

		empDetails.add(new JLabel("Department:"), gp);
		empDetails.add(departmentCombo = new JComboBox<String>(this.parent.department), wrap);
		...
		empDetails.add(fullTimeCombo = new JComboBox<String>(this.parent.fullTime), wrap);
	...
	public boolean checkInput() {
		...
		if (surnameField.getText().isEmpty()) {
			surnameField.setBackground(colour);
			valid = false;
		}// end if
		...
		if (genderCombo.getSelectedIndex() == 0) {
			genderCombo.setBackground(colour);
			valid = false;
		}// end if
		if (departmentCombo.getSelectedIndex() == 0) {
			departmentCombo.setBackground(colour);
			valid = false;
		}// end if
		try {// try to get values from text field
			Double value = Double.parseDouble(salaryField.getText());
			// check if salary is greater than 0
			if (value < 0) {
				salaryField.setBackground(colour);
				valid = false;
			}// end if
		}// end try
		...
		if (fullTimeCombo.getSelectedIndex() == 0) {
			fullTimeCombo.setBackground(colour);
			valid = false;
		}// end if
		return valid;
	}// end checkInput
